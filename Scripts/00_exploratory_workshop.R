...
scores <- read_delim("round1/output/assessments_1stRound.csv")[,-1]
...
techs_new$technology[22] <- "Social media have"
techs_new$technology[1] <- "3D technology to improve CS experience"
techs_new$technology[23] <- "Social media mining has"
...
write.csv(temp, "round2/output/assessments_2ndRound.csv") # additional assessments only
bind_rows(scores, temp) -> temp
write.csv(temp, "round2/output/workshop_individual_ass.csv") # all the individual assessments
# dim(temp)
...
write.csv(summary_coders_tech, "round2/output/summary_coders_tech.csv")
...
ggsave("round2/summary_coders_tech.tiff",
...
ggsave("round2/summary_unknowns_detailed.tiff",
       dpi=300, compression = 'lzw')
...
scores_num_long %>% 
  ...
  geom_violin() 
ggsave("round2/criteria_violin.tiff",
       dpi=300, compression = 'lzw')
# balloon plots
scores_num_long %>% 
...
ggsave("round2/criteria_balloon.tiff",
       dpi=300, compression = 'lzw')
...
while (i <= dim(techs)[1]) {
  ...
  scores_num_long %>%
    ...
    geom_violin()
  ggsave(paste("round2/figs_techs/",tech_plot,"_violin.tiff",sep=""),
         dpi=300, compression = 'lzw')
  ...
  ggballoonplot(pp, x = "criterion", y = "rank",
    ...
    scale_y_continuous(limits = c(1,5)) +
    labs(title = tech_plot)
  ggsave(paste("round2/figs_techs/",tech_plot,"_balloon.tiff",sep=""),
         dpi=300, compression = 'lzw')
...
write.csv(irr_table, "round2/output/irr_table_workshop_individual_ass.csv")
