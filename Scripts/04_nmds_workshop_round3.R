...
ggsave("round3_consensus_building/figs/ordination.tiff",
...
while (i <= dim(techs)[1]) {
  ...
  ggsave(paste("round3_consensus_building/figs/ordination_",tech_plot,".tiff",sep=""), 
                dpi=300, compression = 'lzw')
...
ggsave("round3_consensus_building/figs/ordination_centers.tiff",
       dpi=300, compression = 'lzw')
...
plot2 + 
  geom_text(data = NMDS.mean.tech, aes(label = group),
            position = position_dodge(width=0.9),  size=2)
ggsave("round3_consensus_building/figs/ordination_centers_criteria.tiff",
       dpi=300, compression = 'lzw')
