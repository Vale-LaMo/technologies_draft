...
temp <- read.csv("round2/output/workshop_individual_ass.csv")[,-1]
...
# write.csv(average_scores_criteria, "round2/output/average_scores_criteria.csv")
# write_xlsx(average_scores_criteria, "round2/output/average_scores_criteria.xlsx")
...
# ggsave("round2/figs_ranking/summary_ranking.tiff",
...
average_scores_tot %>% 
  ...
  theme(legend.title = element_blank())
# ggsave("round2/figs_ranking/summary_ranking_application.tiff",
#        dpi=300, compression = 'lzw')
...
average_scores_tot %>% 
  ...
  theme(legend.title = element_blank())
# ggsave("round2/figs_ranking/summary_ranking_engagement.tiff",
#        dpi=300, compression = 'lzw')
...
average_scores_tot %>% 
  ...
  theme(legend.title = element_blank())
# ggsave("round2/figs_ranking/summary_ranking_newinfo.tiff",
#        dpi=300, compression = 'lzw')
...
average_scores_tot %>% 
  ...
  theme(legend.title = element_blank())
# ggsave("round2/figs_ranking/summary_ranking_improvement.tiff",
#        dpi=300, compression = 'lzw')
...
average_scores_tot %>% 
  ...
  scale_fill_viridis(discrete = TRUE, option = "D", begin = 0, end = 1,
                     labels = c("Application", "Engagement", "Data improvement", "New information")) +
  coord_flip() + 
  # theme_minimal() + 
  labs(x = "Technology", y = "Overall score") +
  theme_ipsum_ps(axis_title_size = 10, axis = TRUE, axis_text_size = 6) +
  theme(legend.title = element_blank(), legend.position = "bottom") -> ranking_plot
ranking_plot

tiff("figs/ranking_plot.tiff",
